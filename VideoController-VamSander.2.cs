...
namespace VamSander {
	public class VideoController : MVRScript
	{
		...
		void CreateUI()
		{
			...
			// radius
			jsonOffset = new JSONStorableFloat("Distance", 5, OffsetCallback, 1, 100, true, true);
			...
			// play all
			JSONStorableAction jsonPlay = new JSONStorableAction("Play", PlayCallback);
			RegisterAction(jsonPlay);
		...
		void SetScale(VideoPlayer player, float multiplier)
		{
			...
			player.transform.localScale = new Vector3(scale.x * multiplier, scale.y * multiplier, scale.z * multiplier);
		}
		...
		*/
		...
		void Update() {
			try {
				...
				bool freezeState = (SuperController.singleton.freezeAnimation || pauseVideo);
				if (freezeState != lastFreezeState)
				{
					StartStopAll(!freezeState);
